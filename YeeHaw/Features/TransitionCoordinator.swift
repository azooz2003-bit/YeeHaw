...
class TransitionCoordinator: NSObject {
    ...
    private func animateHomeToGame(_ homeVC: HomeViewController, _ gameVC: GameViewController, using context: UIViewControllerContextTransitioning) {
        guard
            let sequenceBoxSnapshot = gameVC.sequenceBox.snapshotView(afterScreenUpdates: true),
            let gameInfoViewSnapshot = gameVC.gameInfoView.snapshotView(afterScreenUpdates: true)
        else {
            print("Home => Game snapshots not found.")
            return
        }
